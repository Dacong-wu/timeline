...
<template>
  <div ref="main" class="flex flex-col text-xl">
    <div
    ...
    >
      <NuxtLink v-if="item.path" :to="item.path">
        <div
          ...
          :class="{
            'bg-red-500 after:-left-5 ml-3.5 hover:bg-red-600 after:bg-red-500 translate-x-4':
              index % 2 === 0,
            'bg-orange-500 hover:bg-orange-600 after:-right-5 mr-3.5 items-end after:bg-orange-500 -translate-x-4':
              index % 2 !== 0,
          }"
        >
          <p>{{ item.name }}</p>
          <p class="text-sm">
            {{ item.time }}
            <span class="text-2xl">{{ getDayNumber(item.time) }}</span>
          </p>
        </div>
      </NuxtLink>
      <div
